...
x = train.drop(['target'], axis=1)[:100000]
y = np.array(train['target'])[:100000]
...
parameters={'base_estimator__criterion' : base_estimator__criterion,'base_estimator__splitter':base_estimator__splitter }


DTC = DecisionTreeClassifier(max_depth=13,max_features='sqrt',min_samples_split=10)

ABC = AdaBoostClassifier(base_estimator = DTC,n_estimators=100)

# run grid search
model = GridSearchCV(ABC, param_grid=parameters, scoring = 'roc_auc')
model.fit(x, y)
...
print 'grid score :',model.grid_scores_
...
plt.plot(fpr,tpr,label="AdaBoostClassifier ROC, auc="+str(auc))
plt.legend(loc=0)
plt.savefig("ROC_curve_AdaBoostClassifier.png")
